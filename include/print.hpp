...
struct _enable_if_glmvec<
    T, U,
    std::enable_if_t<
        std::is_same_v<decltype(T()[0] * T()), T> && std::is_same_v<decltype(T() * T()[0]), T> &&
        ...
        std::is_same_v<decltype(T() == T()), bool> && std::is_same_v<decltype(T(T()[0])), T> &&
...
struct _enable_if_glmmat<
    ...
    typename _enable_if_glmvec<
        ...
        typename _enable_if_glmvec<
            typename T::row_type,
            std::enable_if_t<std::is_same_v<decltype(T()[0] * T()), typename T::row_type> &&
                             std::is_same_v<decltype(T() * T()[0]), typename T::col_type> &&
                             std::is_same_v<decltype(T()[0][0] * T()), T> &&
                             std::is_same_v<decltype(T()[0][0]), typename T::value_type&>>>::type>::
        type> {
    using type = U;
};
...
template <class T, class U>
struct _enable_if_has_print<
    T, U, std::void_t<decltype(std::declval<T const&>().do_print(std::declval<std::ostream&>()))>> {
    using type = U;
};
...
struct _printer<T, typename _enable_if_has_print<
    ...
    static void _unrolled_print(std::ostream& os, T const& t, std::index_sequence<Is...>) {
        os << "{";
        ((_printer<_rmcvref_t<std::tuple_element_t<Is, T>>>::print(os, std::get<Is>(t)),
          os << ", "),
         ...);
...
template <class T>
struct _printer<T,
                typename _enable_if_has_print<T, typename _enable_if_glmvec<T>::type>::not_type> {
    static void print(std::ostream& os, T const& t) {
        os << "{";
        bool once = false;
...
template <class T>
struct _printer<T,
                typename _enable_if_has_print<T, typename _enable_if_glmmat<T>::type>::not_type> {
    static void print(std::ostream& os, T const& t) {
        os << "{";
        bool once = false;
        for (typename T::length_type i = 0; i < t.length(); i++) {
...
struct _printer<std::nullptr_t, void> {
    static void print(std::ostream& os, std::nullptr_t const&) { os << "nullptr"; }
};

template <>
struct _printer<std::nullopt_t, void> {
    static void print(std::ostream& os, std::nullopt_t const&) { os << "nullopt"; }
};

template <>
struct _printer<std::monostate, void> {
    static void print(std::ostream& os, std::monostate const&) { os << "monostate"; }
};
